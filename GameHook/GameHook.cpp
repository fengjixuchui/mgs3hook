...
// patch toggle bools
bool GameHook::cursorForceHidden_toggle = false;
bool GameHook::forceModelsVisible_toggle = false;
bool GameHook::infiniteJumps_toggle = false;
bool GameHook::infiniteAirCombos_toggle = false;
bool GameHook::forceCharSelect_toggle = false;
bool GameHook::spoiler_toggle = false;
bool GameHook::takeNoDamage_toggle = false;
bool GameHook::dealNoDamage_toggle = false;
bool GameHook::infiniteMagic_toggle = false;
int GameHook::forceCharSelect_num = 0;

// gui
uintptr_t GameHook::playerActor = 0x26F9560;
uintptr_t GameHook::playerLevel = 0x4374A84;
uintptr_t GameHook::playerXP = 0x4374A90;
uintptr_t GameHook::playerGold = 0x4374ADC;
uintptr_t GameHook::playerHP = 0x4374A6C;
uintptr_t GameHook::playerMP = 0x4374A78;
uintptr_t GameHook::playerZone = 0x4374A24;
uintptr_t GameHook::playerName = 0x4374A4C;

// system
uintptr_t GameHook::enableInput = 0x443E48E;
uintptr_t GameHook::showCursor = 0x443E48F; // 1 byte later
uintptr_t GameHook::currentCharacter = 0x4374A48;
uintptr_t GameHook::playerEndgame = 0x435AF40;

// inventory
std::vector<std::pair<std::string, uintptr_t>> GameHook::recoveryInventoryVec;
std::vector<std::pair<std::string, uintptr_t>> GameHook::cultivationInventoryVec;
std::vector<std::pair<std::string, uintptr_t>> GameHook::fishingInventoryVec;
std::vector<std::pair<std::string, uintptr_t>> GameHook::materialsInventoryVec;
std::vector<std::pair<std::string, uintptr_t>> GameHook::keyInventoryVec;
...
// patches
void GameHook::cursorForceHidden(bool enabled) { // disables the game displaying the cursor when using a gamepad
	if (enabled) {
		GameHook::_nop((char*)(GameHook::_baseAddress + 0x3D48E9), 7);
		*(bool*)(GameHook::_baseAddress + showCursor) = false; // hide cursor
	}
	else {
		GameHook::_patch((char*)(GameHook::_baseAddress + 0x3D48E9), (char*)"\x40\x88\xB3\x8F\x00\x00\x00", 7);
		*(bool*)(GameHook::_baseAddress + showCursor) = true; // show cursor
	}
}

void GameHook::forceModelsVisible(bool enabled) {
	if (enabled)
		GameHook::_nop((char*)(GameHook::_baseAddress + 0x156E6A), 8);
	else
		GameHook::_patch((char*)(GameHook::_baseAddress + 0x156E6A), (char*)"\xF3\x0F\x10\x8B\x80\x15\x00\x00", 8);
}

void GameHook::infiniteJumps(bool enabled) {
	if (enabled)
		GameHook::_nop((char*)(GameHook::_baseAddress + 0x6A007A), 2);
	else
		GameHook::_patch((char*)(GameHook::_baseAddress + 0x6A007A), (char*)"\x84\xC0", 2);
}

void GameHook::infiniteAirCombos(bool enabled) {
	if (enabled)
		GameHook::_nop((char*)(GameHook::_baseAddress + 0x6C28D8), 7);
	else
		GameHook::_patch((char*)(GameHook::_baseAddress + 0x6C28D8), (char*)"\xFF\x84\x81\x2C\x61\x01\x00", 7);
}

void GameHook::takeNoDamage(bool enabled) {
	if (enabled)
		_nop((char*)(GameHook::_baseAddress + 0x3BE25D), 5);
	else
		_patch((char*)(GameHook::_baseAddress + 0x3BE25D), (char*)"\x44\x89\x44\x81\x4C", 5);
}

void GameHook::dealNoDamage(bool enabled) {
	if (enabled)
		_nop((char*)(GameHook::_baseAddress + 0x2A63BE), 6);
	else
		_patch((char*)(GameHook::_baseAddress + 0x2A63BE), (char*)"\x89\xBB\xEC\x02\x00\x00", 6);
}

void GameHook::infiniteMagic(bool enabled) {
	if (enabled)
		_nop((char*)GameHook::_baseAddress + 0x3BE2BE, 6);
	else
		_patch((char*)GameHook::_baseAddress + 0x3BE2BE, (char*)"\xF3\x0F\x11\x54\x81\x58", 6);
}

// static values
void GameHook::stealCursor(bool enabled) {
	if (enabled) {
		*(bool*)(GameHook::_baseAddress + enableInput) = false; // disable game input
		*(bool*)(GameHook::_baseAddress + showCursor) = true; // show cursor
	}
	else {
		*(bool*)(GameHook::_baseAddress + enableInput) = true;  // enable game input
		*(bool*)(GameHook::_baseAddress + showCursor) = false; // hide cursor
	}
}

void GameHook::forceEndgameStats(bool enabled) {
	*(bool*)(GameHook::_baseAddress + playerEndgame) = true; // playerEndgame in main.cpp
}

void GameHook::forceCharSelect(int character) {
	*(int*)(GameHook::_baseAddress + currentCharacter) = character;
}

// dev functions
void GameHook::getInventoryAddresses(void) { // called in InitHook
	// Recovery
...
void GameHook::_hook(void) {
	...
	if (ID <= 0)
		return;
	GameHook::_pID = ID;
	GameHook::_baseAddress = GameHook::_getModuleBaseAddress(ID, "NieR Replicant ver.1.22474487139.exe");
